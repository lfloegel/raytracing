...
class aabb {
    public:
        ...
        bool hit(const ray& r, double tmin, double tmax) const {
            for (int a = 0; a < 3; a++) {
                ...
                auto t1 = fmax((_min[a] - r.origin()[a]) / r.direction()[a],
                               (_max[a] - r.origin()[a]) / r.direction()[a]);
                tmin = fmax(t0, tmin); //taking max of min for smaller bounding volume 
                tmax = fmin(t1, tmax);
                if (tmax <= tmin)
                    return false;
            }
        ...
        */
        ...
        //optimized version - Andrew Kensler 
...
aabb surrounding_box(aabb box0, aabb box1) { //computes bounding box of two boxes 
