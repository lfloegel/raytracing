...
color ray_color(const ray& r, const hittable& world, int depth) { 
    ...
    if (world.hit(r, 0.001, infinity, rec)) { //if intersects with hittable object 
        ...
        if (rec.mat_ptr->scatter(r, rec, attenuation, scattered))
            return attenuation * ray_color(scattered, world, depth-1); //return color 
...
int main() {
   ...
   /*
    hittable_list world; //adding spheres to world 
    ...
    */ 
    ...
    camera cam(lookfrom, lookat, vup, 20, aspect_ratio, aperture, dist_to_focus); //first: camera position, fov: zooms 
